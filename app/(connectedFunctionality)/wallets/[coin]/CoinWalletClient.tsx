    ...
    >
      ...
      {isFetchingWallet ? (
        <div className="flex-1 flex flex-col items-center justify-center py-24">
          ...
          <span className="mt-4 text-sm" style={{ color: COLORS.textGray }}>
            Loading wallet...
          </span>
        </div>
      ) : (
                ...
                {coin.toUpperCase() === 'USDT' ? (
                  <>
                    ...
                    <select
                      ...
                      style={{ background: "#23232a" }}
                    >
                      {/* Add other coins you want to allow conversion to */}
                      <option>BTC</option>
                      <option>ETH</option>
                      <option>BNB</option>
                      <option>SOL</option>
                    ...
                    <button
                      className="w-full py-3 bg-gradient-to-r from-green-500 to-green-400 text-white rounded-xl font-bold shadow-lg hover:scale-105 transition disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={isConverting}
                      onClick={async () => {
                        const amt = parseFloat(convertAmount || '0');
                        if (!amt || amt <= 0 || isConverting) return;

                        setIsConverting(true);
                        try {
                          const res = await fetch('/api/convert-from-usdt', {
                            method: 'POST',
